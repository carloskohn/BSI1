...
def situacao_aluno(nota1, nota2, nota3, faltas, aulas_ministradas):
  ...
  :return: retorna situação do aluno como string
  ...
  media_prova = (nota1 + nota2 + nota3) / 3
  frequencia = 100-((faltas * 100) / aulas_ministradas)

  if frequencia < 75:
      return 'RF'
  if media_prova >= 7 and frequencia >=75:
      return 'AP'
  else:
      return 'EF'
...
def conta_combustivel(qtde_litros, tipo_combustivel, tipo_pagamento):
  ...
  if tipo_combustivel == 'a':
      preco_combustivel = 3.159
  elif tipo_combustivel == 'g':
      preco_combustivel = 3.739
  else:
      preco_combustivel = 3.090

  preco_total = qtde_litros * preco_combustivel
