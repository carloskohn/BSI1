...
def media_final_aprovado_reprovado(p1, p2, ep1, ep2):
    ...
    nota_parcial_1 = (p1 * .7) + (ep1 * .3)
    nota_parcial_2 = (p2 * .7) + (ep2 * .3)
...
def excesso_peso_peixes(peso_peixes_kg, peso_limite):
    ...
    excesso_peixes = peso_peixes_kg - peso_limite
    if excesso_peixes > 0:
        return round(excesso_peixes, 2)
    else:
        return 0
...
def tinta(metros_pintar):
    ...
    if metros_pintar % 54 == 0:
        quant_latas = metros_pintar/54
    else:
        quant_latas = int(metros_pintar / 54) + 1
...
def acrescimo_nota_bb(nota_sozinho, nota_com_ajuda):
    ...
    if nota_com_ajuda < nota_sozinho:
        ...
        acrecimo_nota = (nota_com_ajuda-nota_sozinho) / 4

    return round(acrecimo_nota, 1)
